...
class MileageProgress : View {
    ...
    private var mMaxProgress = 100//默认最大进度

    private var mDefaultProgress = 0//默认进度为0

    private var mWarnProgress = 30//告警进度

    private var mIsDimProgress = true//模糊进度

    private var mDefaultHeight = 20//默认的高度
    ...
    constructor(
        ...
        context.obtainStyledAttributes(attrs, R.styleable.MileageProgress)
            .apply {
                //背景颜色
                mBackgroundColor =
                    getColor(R.styleable.MileageProgress_mp_background, mBackgroundColor)
                //背景颜色
                mBackgroundColor =
                    getColor(R.styleable.MileageProgress_mp_background, mBackgroundColor)
                //角度的宽
                mAngleWidth = getDimension(R.styleable.MileageProgress_mp_angle_width, mAngleWidth)
                //获取定义的颜色数组
                val resourceId = getResourceId(R.styleable.MileageProgress_mp_anamorphism_color, 0)
                if (resourceId != 0) {
                    mColorArray = resources.getIntArray(resourceId)
                }

                //模糊进度
                mIsDimProgress = getBoolean(R.styleable.MileageProgress_mp_is_dim_progress, true)

                //最大进度
                mMaxProgress = getInt(R.styleable.MileageProgress_mp_max_progress, mMaxProgress)

                //默认进度
                mDefaultProgress =
                    getInt(R.styleable.MileageProgress_mp_default_progress, mDefaultProgress)

                //警告进度
                mWarnProgress =
                    getInt(R.styleable.MileageProgress_mp_warn_progress, mWarnProgress)
                //告警颜色
                mWarnBackgroundColor =
                    getColor(R.styleable.MileageProgress_mp_warn_background, mWarnBackgroundColor)
            }
        initData(context)
    }
    ...
    private fun canvasParallelogram(canvas: Canvas) {
        ...
        val defaultProgress = mDefaultProgress.toFloat() / mMaxProgress.toFloat()//当前进度

        if (mDefaultProgress <= mWarnProgress) {
            //如果低于某一个阀门，就标注为告警颜色
